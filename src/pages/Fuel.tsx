...
const ManualEntryModal = ({
  employees, defaultMonthYear, onClose, onSaved,
  editRow,
}: {
  employees: Employee[];
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md border border-border/50">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-lg font-bold text-foreground">{editRow ? 'تعديل السجل' : 'إدخال يدوي'}</h2>
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border border-border/50">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <h2 className="text-lg font-bold text-foreground">استيراد Excel من GPS</h2>
          <button onClick={onClose} className="p-2 hover:bg-muted rounded-lg text-muted-foreground"><X size={16} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-sm text-muted-foreground mt-1">ارفع ملف Excel من منصة GPS يحتوي على أسماء المناديب والكيلومترات</p>
                <p className="text-xs text-muted-foreground mt-2">xlsx, csv — حتى 20MB</p>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">تم اكتشاف <strong>{headers.length}</strong> عمود و <strong>{rawData.length}</strong> صف. اختر الأعمدة المقابلة:</p>
                  ...
                  <div key={f.key}>
                    ...
                    >
                      <SelectTrigger className="h-9 text-sm">
                        <SelectValue placeholder={f.required ? 'مطلوب' : 'اختياري'} />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <div className="space-y-4">
              ...
              <div className="border border-border rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rows.map((row, i) => {
                      const emp = row.manual_employee_id
                        ? employees.find(e => e.id === row.manual_employee_id)
                          ...
                          <td className="px-3 py-2">
                            {isMatched ? (
                              ...
                              <Select
                                ...
                                onValueChange={v => setRows(rs => rs.map((r, j) => j === i ? { ...r, manual_employee_id: v } : r))}
                              >
                                <SelectTrigger className="h-7 text-xs border-warning/50">
                                  <SelectValue placeholder="اختر يدوياً..." />
                                </SelectTrigger>
...
const FuelPage = () => {
  ...
  // Fetch employees
  useEffect(() => {
    supabase.from('employees').select('id, name, personal_photo_url').eq('status', 'active').order('name')
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'الوقود');
    XLSX.writeFile(wb, `الوقود_والكيلومترات_${selectedMonth}_${selectedYear}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={<Package size={22} />} label="إجمالي الطلبات" value={totalOrders.toLocaleString()} sub="هذا الشهر" />
      </div>
      ...
      <div className="bg-card rounded-xl border border-border/50 shadow-sm p-4 flex flex-wrap gap-3 items-center">
        ...
        <Button variant="outline" className="gap-2" onClick={() => setShowImport(true)}>
          <Upload size={15} /> 📥 استيراد Excel من GPS
        </Button>
                  ...
                  {filtered.map(row => {
                    ...
                    return (
                      <tr key={row.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-muted-foreground">{orders > 0 ? orders.toLocaleString() : '—'}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 justify-center">
                            <button
                              onClick={() => { setEditRow(row); setShowManual(true); }}
