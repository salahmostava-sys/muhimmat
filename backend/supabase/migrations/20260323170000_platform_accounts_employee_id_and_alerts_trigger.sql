...
-- Add optional fields so alerts can carry rich listing details
ALTER TABLE public.alerts
  ADD COLUMN IF NOT EXISTS message TEXT,
  ADD COLUMN IF NOT EXISTS details JSONB;

-- Trigger: create alerts when an employee becomes absconded/terminated
-- (used by Platform Accounts + Employee Tiers requirements)
...
BEGIN
  ...
  IF (NEW.sponsorship_status IS DISTINCT FROM OLD.sponsorship_status)
    ...
    -- For terminated: only create alert if at least one platform account is linked
    ...
    -- Vehicle assignments (active ones)
    SELECT
      COUNT(*)::int,
...
DROP TRIGGER IF EXISTS trg_employee_sponsorship_alerts ON public.employees;
