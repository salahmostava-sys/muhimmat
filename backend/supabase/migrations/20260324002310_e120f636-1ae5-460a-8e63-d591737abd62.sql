...
DROP POLICY IF EXISTS "Active users can view platform_accounts"
...
DROP POLICY IF EXISTS "Admin/operations can manage platform_accounts"
  ON public.platform_accounts;
...
DROP POLICY IF EXISTS "Active users can view account_assignments"
...
DROP POLICY IF EXISTS "Admin/operations can manage account_assignments"
  ON public.account_assignments;
...
DROP POLICY IF EXISTS "Active users can view vehicle_mileage_daily"
  ON public.vehicle_mileage_daily;
CREATE POLICY "Active users can view vehicle_mileage_daily"
  ON public.vehicle_mileage_daily FOR SELECT
  USING (is_active_user(auth.uid()));

DROP POLICY IF EXISTS "Admin/operations can manage vehicle_mileage_daily"
  ON public.vehicle_mileage_daily;
...
DROP TRIGGER IF EXISTS update_vehicle_mileage_daily_updated_at
  ON public.vehicle_mileage_daily;
...
DROP POLICY IF EXISTS "Active users can view locked_months"
  ON public.locked_months;
CREATE POLICY "Active users can view locked_months"
  ON public.locked_months FOR SELECT
  USING (is_active_user(auth.uid()));

DROP POLICY IF EXISTS "Admin/finance can manage locked_months"
  ON public.locked_months;
